...
class EwayBill(models.Model):
    ...
    def _get_last_generate_request(self):
        last_generate_request = self.env["l10n.in.ewaybill.transaction"]
        if self.move_id:
    ...
    def _get_response_json_dict(self):
        self.ensure_one()
    ...
    def _prepare_compute_request_json_template_values(self):
        ...
        if self.move_id:
            values.update(
                self.env["account.edi.format"]._l10n_in_prepare_edi_tax_details(
                    self.move_id
                )
                or {}
            )
    ...
    def _get_ewaybill_templates(self):
        self.ensure_one()
        return TEMPLATES.get(self.request_type)
    ...
    def _compute_request_json(self):
        ...
        for transaction in self:
            template = transaction._get_ewaybill_templates()
            if template:
    ...
    def _get_document_company(self):
        self.ensure_one()
    ...
    def submit(self):
        ...
        service = WaybillService.get_service(self._get_document_company())
